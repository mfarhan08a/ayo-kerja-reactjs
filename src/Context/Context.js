...
export const Provider = (props) => {
  ...
  const handleFilter = (e) => {
    ...
    axios.get(`https://dev-example.sanbercloud.com/api/job-vacancy`).then(({ data }) => {
      ...
      let result = fetchResult.filter((res) => {
        return res.job_type?.toLowerCase() === filterIn.job_type?.toLowerCase() || res.company_city?.toLowerCase() === filterIn.company_city?.toLowerCase() || res.company_name?.toLowerCase() === filterIn.company_name?.toLowerCase();
      });
  ...
  const functionDelete = (id) => {
    axios
      .delete(`https://dev-example.sanbercloud.com/api/job-vacancy/${id}`, { headers: { Authorization: "Bearer " + Cookies.get("token") } })
      .then(() => {
        setFetchStatus(true);
      })
      .catch((e) => {
